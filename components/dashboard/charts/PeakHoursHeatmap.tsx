  ...
  return (
    <div className="bg-card border border-border rounded-xl">
      <div className="px-4 lg:px-5 py-4 border-b border-border">
        <h3 className="font-display text-[15px] text-foreground">Peak Hours</h3>
        <p className="text-[11px] text-muted-foreground mt-0.5">Booking density by day and hour</p>
      </div>
        ...
        <div className="overflow-x-auto -mx-4 px-4 lg:mx-0 lg:px-0">
          <div className="min-w-[420px]">
            <div className="flex mb-1">
              ...
              {hours.map((hour) => (
                <div key={hour} className="flex-1 text-center font-data text-[9px] lg:text-[10px]" style={{ color: "#9c9184" }}>
            ...
            {days.map((day, dayIndex) => (
              <div key={day} className="flex items-center mb-1">
                <div className="w-8 lg:w-10 text-[10px] lg:text-[11px] font-medium flex-shrink-0" style={{ color: "#9c9184" }}>{day}</div>
                <div className="flex-1 flex gap-0.5">
                  {hours.map((hour) => {
                    const cell = data.find((c) => c.day === dayIndex && c.hour === hour);
                    return (
                      <div key={hour} className="flex-1 h-6 lg:h-7 rounded-sm transition-colors" style={{ backgroundColor: getIntensityColor(cell?.count ?? 0) }} title={`${day} ${hour}:00 — ${cell?.count ?? 0} bookings`} />
                    );
                  })}
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-1 mt-3">
              ...
              {INTENSITY_COLORS.map((color) => (
                <div key={color} className="w-3.5 h-3.5 lg:w-4 lg:h-4 rounded-sm" style={{ backgroundColor: color }} />
              ))}
